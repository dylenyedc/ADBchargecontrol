...
- Web UI 增加电池仪表盘，可显示充放电电流、电压、温度、健康度、电池类型、电源接入、charge counter、最大充电能力
...
- 电池状态：电量、温度、充电状态、plugged 信息、电流、电压、charge counter、最大充电能力、电池类型等
- 当时采用的策略配置
- 策略决策与原因
- 是否实际执行了控制动作
- 最近执行动作结果
...
Web UI 的“历史记录”区域可在 1h、2h、4h、12h、24h 之间切换，并绘制电量曲线：曲线颜色表示温度黄到红，曲线下方面积只表示当时充电状态，绿色为充电中、无填充为未充电、黄色为未知。鼠标悬停可查看当时的电流、电压、健康度、电池类型和策略依据。
...
如果 `serial` 配置为 TCP 地址，服务在 ADB 健康检查失败后会自动尝试：
