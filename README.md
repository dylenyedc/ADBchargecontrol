...
- `data/config.json`：真实设备 serial、ADB 地址、root 写入路径等本机配置
- `data/state.json`：最近一次运行状态
- `data/history.jsonl`：电池历史、时间戳和设备状态
- `skill.md`：本机维护笔记

公开仓库建议只提交 `data/config.example.json`。如果真实配置已经进入过 Git 历史，单纯在新提交里删除并不能从历史中抹掉；发布到 GitHub 前应使用干净的新仓库、orphan 分支，或 `git filter-repo` 之类工具重写历史。
